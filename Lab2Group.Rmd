...
### Second Data Visualization Improvement
* Chart 2.16: Differences in perceived knowledge of science between men and women by region on page 41 could be improved a little. This chart is meant to be showing the discrepancy between knowledge of science between genders across the world.
* % difference in perceived knowledge of science, region
* x = % difference, y = region
* I would call this a barchart
* The graph actually does a pretty good job of displaying the data it says it is portraying but I believe a lot more could be done with this graph to make it less cluttered and also display more info at the same time. The graph I have created not only displays the difference between men and women for each region but also the individual percentages for each gender. I also consolidated the regions to make the graph more readable. Overall I believe this graph shows more info in a neater orientation.

```{r}
male <- countries %>%
  group_by(WP5) %>%
  filter(Gender == 2) %>%
  mutate(total = percent_yes(Q1),
         percent = (sum(total, na.rm = TRUE) / length(WP5)) * 100
  )
...
male <- male %>% 
  tibble::as_tibble() %>%
  group_by(regions) %>%
  summarise_at(vars(percent), list(Avg = mean))
...
final <- merge(male, female, by = "regions")

final <- final %>% 
  mutate(
  diff = Avg.x - Avg.y)
colnames(final) <- c("Regions", "Male", "Female", "Difference")
```
...
final %>%
  ggplot() +
  geom_bar(aes(y=Male, x = Regions), stat="identity", alpha=.8, fill='lightblue', color='lightblue4') +
  geom_bar(aes(y=Female, x = Regions), stat="identity", alpha=.5, fill='pink', color='red') +
       geom_text(aes(label = round(Difference,2), x=Regions, y=Male), vjust = 1.5, colour = "black", size = 1.5) +
  labs(title = "Percieved Knowledge of Science by Region and Gender", x = "Region", y = "% answer a lot / some") +
  theme_classic()
```
